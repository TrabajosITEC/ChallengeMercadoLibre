...
export default function CardProductos({ info }) {
  const { Dolar, moneda ,Modo} = useContext(ModeContext)
  ...
  const itemTemplate = (product, index) => {
    ...
    return (
      <div className="col-11" key={product.id}>
        <div style={ Modo ? {}: {backgroundColor:"rgb(37, 41, 37)"}} className={classNames('flex flex-column xl:flex-row xl:align-items-start p-4 gap-4', { 'border-top-1 surface-border': index !== 0 })}>
          ...
          <div className="flex flex-column sm:flex-row justify-content-between align-items-center xl:align-items-start flex-1 gap-4">
            <div className="col-9 flex flex-column align-items-center sm:align-items-start gap-3">
                <h3 style={ Modo ? {}: {color:"white"}} className='marcaProducto'>
                  {product.attributes.find(atr => atr.id === "BRAND" )? product.attributes.find(atr => atr.id === "BRAND" ).value_name : "" }
                </h3>
              <Button style={ Modo ? {}: {color:"white"}} className='custom-link-button'  label={product.title} onClick={() => handleDetalleProducto(product.id)}/>
           
              <div className="flex align-items-center gap-3">
   
                <span style={ Modo ? {}: {color:"white"}} className="flex align-items-center gap-2">
            ...
            <div className="col-3 flex sm:flex-column align-items-center sm:align-items-end gap-3 sm:gap-2">
              <span style={ Modo ? {}: {color:"white"}} className="mt-5 text-xl font-semibold">  
                {
